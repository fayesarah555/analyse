...
# Initialiser une session Spark avec support Iceberg
spark = SparkSession.builder \
    .appName("GameSalesDataProcessing") \
    .config("spark.sql.extensions", "org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions") \
    .config("spark.sql.catalog.game_catalog", "org.apache.iceberg.spark.SparkCatalog") \
    .config("spark.sql.catalog.game_catalog.type", "hadoop") \
    .config("spark.sql.catalog.game_catalog.warehouse", "/data/warehouse") \
    .getOrCreate()
...
full_sales_df = full_sales_df.withColumn("year", year(col("sale_date")))
full_sales_df = full_sales_df.withColumn("month", month(col("sale_date")))

full_sales_df.writeTo("game_catalog.default.game_sales_full") \
...
monthly_platform_sales.writeTo("game_catalog.default.monthly_platform_sales").createOrReplace()
...
game_performance.writeTo("game_catalog.default.game_performance").createOrReplace()
...
customer_segments = spark.sql("""
    ...
    FROM game_sales_full
    GROUP BY segment, loyalty_status, age_group, sale_country
    ORDER BY total_spent DESC
""")

# customer_segments.write.mode("overwrite").parquet("/data/processed/customer_segments")
customer_segments.writeTo("game_catalog.default.customer_segments").createOrReplace()
...
channel_performance.writeTo("game_catalog.default.channel_performance").createOrReplace()
...
genre_trends.writeTo("game_catalog.default.genre_trends").createOrReplace()
